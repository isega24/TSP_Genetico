    ...
    */
    ...
    while(posibles_hijos.size() < evolucion_individuos[0].size()){
        ...
        if(rand()*1.0/INT_MAX < prob_procrear){
            hijo = evolucion_individuos[evolucion_individuos.size()-1][i].procrea(evolucion_individuos[evolucion_individuos.size()-1][j]);
    ...
    double total = 0;
    for(int i = 0; i < evolucion_individuos[0].size()){
